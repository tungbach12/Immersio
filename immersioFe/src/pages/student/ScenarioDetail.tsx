...
export default function ScenarioDetail() {
  ...
  const handleFinishLesson = async () => {
    ...
    try {
      ...
      // Auto-trigger dynamic dynamic custom flashcards generation using all three default options
  ...
  if (loadingScenario) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white">
        ...
        <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-4">Connecting to simulation server...</p>
      </div>
    );
  }
            ...
            {!isLoadingFeedback && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <Button
                    onClick={() => toggleFlashcardOption("grammar")}
                    disabled={isLoadingFlashcards}
                    className={cn(
                      "h-14 rounded-2xl font-bold transition-all border border-indigo-500/20",
                      selectedFlashcardOptions.includes("grammar") ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/35" : "bg-white/5 text-white/70 hover:bg-white/10"
                    )}
                  >
                    {selectedFlashcardOptions.includes("grammar") ? "✓ Grammar" : "Grammar"}
                  </Button>
                  <Button
                    onClick={() => toggleFlashcardOption("vocabulary")}
                    disabled={isLoadingFlashcards}
                    className={cn(
                      "h-14 rounded-2xl font-bold transition-all border border-indigo-500/20",
                      selectedFlashcardOptions.includes("vocabulary") ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/35" : "bg-white/5 text-white/70 hover:bg-white/10"
                    )}
                  >
                    {selectedFlashcardOptions.includes("vocabulary") ? "✓ Vocabulary" : "Vocabulary"}
                  </Button>
                  <Button
                    onClick={() => toggleFlashcardOption("improvement")}
                    disabled={isLoadingFlashcards}
                    className={cn(
                      "h-14 rounded-2xl font-bold transition-all border border-indigo-500/20",
                      selectedFlashcardOptions.includes("improvement") ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/35" : "bg-white/5 text-white/70 hover:bg-white/10"
                    )}
                  >
                    {selectedFlashcardOptions.includes("improvement") ? "✓ Sentence Improvement" : "Sentence Improvement"}
                  </Button>
                </div>
                ...
                <div className="flex justify-center pt-2">
                  <Button
                    ...
                    className="px-8 h-12 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-600/20"
                  >
                    <Sparkles size={16} />
                    Generate Custom Cards ({selectedFlashcardOptions.length})
                  </Button>
                </div>
